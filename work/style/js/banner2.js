...
(function($){     
	...
	var defaults={
		...
		'buttonNumer':'<li>{$index}</li>',//{$index}会被替换为为当前页码
	...
	$.hkuc_fs_trans=function(wrapper,opt){
		...
		this.$elements.each(function(index){
			switch(cache['this'].options.type){
				...
				case 'slide':
				default:
					$(this).css({ 'width':'100%', 'overflow':'hidden', 'left':index+'00%', 'position':'absolute','z-index':cache['this'].options.zIndex });
			}
		})
	...
	$.hkuc_fs_trans_init=function(){
		...
		if(this.options['showNumber']){
			...
			var numner_tag=$(this.options['buttonNumer']).prop('nodeName').toLowerCase();
	...
	var hkuc_fs_trans_to=function(param){
		...
		if(this.$elements.filter(':animated').length)return;
